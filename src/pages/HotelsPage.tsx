...
import { 
  ...
  InputAdornment, Link, Card, CardContent, CardActions, 
...
export default function HotelsPage() {
  ...
  const { hotels, addHotel, updateHotel, deleteHotel, uploadHotelImage, uploading } = useHotels();
    ...
    <Box component="main" sx={{ p: 2 }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          ...
          <Box>
            <Typography variant="h4" sx={{ fontWeight: 900, letterSpacing: '-0.5px' }}>Hoteles</Typography>
        ...
        <Stack direction="row" spacing={2}>
          <Button variant="outlined" startIcon={<SaveAltIcon />} onClick={() => exportHotelsToExcel(filteredHotels)} sx={{ borderRadius: 2, fontWeight: 'bold' }}>
            Exportar Excel
          </Button>
          {isAdmin && (
            <Button variant="contained" startIcon={<AddIcon />} onClick={handleOpenAddModal} sx={{ borderRadius: 2, px: 3, fontWeight: 'bold', background: 'linear-gradient(45deg, #FF5722 30%, #FF8A65 90%)', boxShadow: '0 4px 14px rgba(255, 87, 34, 0.3)' }}>
              Añadir Hotel
            </Button>
          )}
        </Stack>
      ...
      <Paper 
        elevation={0}
        sx={{ p: 2, mb: 4, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}
      >
        <TextField
          sx={{ flexGrow: 1, minWidth: '300px' }}
          variant="outlined" size="small" placeholder="Buscar por nombre o ciudad..."
          value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
          InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon color="primary" fontSize="small" /></InputAdornment>, sx: { borderRadius: 2 } }}
        />
        <Divider orientation="vertical" flexItem sx={{ height: 24, mx: 1, opacity: 0.1 }} />
        <FormControlLabel
          control={<Switch checked={showHotelsWithEmployees} onChange={(e) => setShowHotelsWithEmployees(e.target.checked)} size="small" color="primary" />}
          label={<Typography variant="body2" sx={{ fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: 0.5 }}><PeopleIcon fontSize="small" color="primary" /> Con Personal</Typography>}
        />
      </Paper>
      ...
      {filteredHotels.length > 0 ? (
        <Grid container spacing={3}>
          {filteredHotels.map((hotel) => (
            <Grid item key={hotel.id} xs={12} sm={6} md={4} lg={3}>
              <Card sx={{ 
                height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 4, overflow: 'hidden', transition: 'all 0.3s ease',
                backgroundColor: isLight ? '#FFFFFF' : 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)',
                '&:hover': { transform: 'translateY(-8px)', boxShadow: `0 12px 30px -10px ${isLight ? 'rgba(0,0,0,0.1)' : 'rgba(0,0,0,0.5)'}`, borderColor: 'primary.main' }
              }}>
                <Box sx={{ position: 'relative', height: 180, overflow: 'hidden' }}>
                  <S3Image path={hotel.imageUrl} alt={hotel.name} height={180} />
                  
                  {/* CÓDIGO OPERATIVO (Izquierda) */}
                  <Chip 
                    label={hotel.hotelCode} 
                    size="small" 
                    sx={{ 
        ...
        <EmptyState icon={<ApartmentIcon />} title="No se encontraron hoteles" subtitle="Intenta con otro término de búsqueda." />
      )}
      ...
      <FormModal open={isModalOpen} onClose={handleCloseModal} onSave={handleSave} title={currentHotel.id ? "Editar Hotel" : "Añadir Nuevo Hotel"}>
        <HotelForm hotelData={currentHotel} onFormChange={handleFormChange} uploadHotelImage={uploadHotelImage} isEditMode={!!currentHotel.id} uploading={uploading} />
      </FormModal>
    </Box>
  );
}
